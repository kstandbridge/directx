...
namespace UI
{
	...
	private:
		...
		std::function<util::Expected<void>()> onClick;// function to be replaced by a lambda function later on to define what happens when the button is clicked
	...
	public:
		...
		AnimatedButton(std::wstring, graphics::AnimatedSprite* const, std::function<util::Expected<void>()>, unsigned int clickAnimation = 2);
		~AnimatedButton();
		
		// button interaction
		void select();
		void deselect();
		util::Expected<void> click();
		void lock();
		void setOnClickFunction(std::function<util::Expected<void>()> onClickFunction);
